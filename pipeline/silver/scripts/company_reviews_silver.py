    ...
    try:
        ...
        if (
            ...
            raise EnvironmentError(
                "Las variables de entorno de RDS no están configuradas correctamente (DB_HOST, DB_PORT, etc.)."
            )
...
def transform_data(
    ...
    df_dim_industry = (
        df_fact[["industry"]].drop_duplicates().dropna().reset_index(drop=True)
    )
    ...
    # 2. DIM_HEADQUARTERS
    df_dim_hq = (
        df_fact[["headquarters"]].drop_duplicates().dropna().reset_index(drop=True)
    )
    ...
    # 3. DIM_CEO_APPROVAL
    df_dim_ceo = (
        df_fact[["ceo_approval"]].drop_duplicates().dropna().reset_index(drop=True)
    )
...
def load_to_rds(
    ...
    df_dim_industry: pd.DataFrame,
    df_dim_hq: pd.DataFrame,
    df_dim_ceo: pd.DataFrame,
):
    ...
    try:
        with engine.begin() as connection:
            ...
            connection.execute(
                text(f"DROP TABLE IF EXISTS {FACT_REVIEWS_NAME} CASCADE;")
            )
            connection.execute(
                text(f"DROP TABLE IF EXISTS {DIM_INDUSTRY_NAME} CASCADE;")
            )
            connection.execute(text(f"DROP TABLE IF EXISTS {DIM_HQ_NAME} CASCADE;"))
            connection.execute(
                text(f"DROP TABLE IF EXISTS {DIM_CEO_APPR_NAME} CASCADE;")
            )
            ...
            connection.execute(
                text(f"ALTER TABLE {FACT_REVIEWS_NAME} ADD PRIMARY KEY (review_id);")
            )
            connection.execute(
                text(
                    f"ALTER TABLE {DIM_INDUSTRY_NAME} ADD PRIMARY KEY (review_industry_id);"
                )
            )
            connection.execute(
                text(f"ALTER TABLE {DIM_HQ_NAME} ADD PRIMARY KEY (headquarters_id);")
            )
            connection.execute(
                text(
                    f"ALTER TABLE {DIM_CEO_APPR_NAME} ADD PRIMARY KEY (ceo_approval_id);"
                )
            )
            ...
            connection.execute(
                text(
                    f"ALTER TABLE {FACT_REVIEWS_NAME} ADD FOREIGN KEY (company_id) REFERENCES {FACT_COMPANY_INFO_NAME} (company_id);"
                )
            )
            # FKs hacia Dimensiones de Review
            connection.execute(
                text(
                    f"ALTER TABLE {FACT_REVIEWS_NAME} ADD FOREIGN KEY (review_industry_id) REFERENCES {DIM_INDUSTRY_NAME} (review_industry_id);"
                )
            )
            connection.execute(
                text(
                    f"ALTER TABLE {FACT_REVIEWS_NAME} ADD FOREIGN KEY (headquarters_id) REFERENCES {DIM_HQ_NAME} (headquarters_id);"
                )
            )
            connection.execute(
                text(
                    f"ALTER TABLE {FACT_REVIEWS_NAME} ADD FOREIGN KEY (ceo_approval_id) REFERENCES {DIM_CEO_APPR_NAME} (ceo_approval_id);"
                )
            )
...
def main():
    ...
    if df_fact_table.empty:
        print(
            "Pipeline finalizado, no hay datos válidos después del JOIN y la limpieza."
        )
        return
