...
def transform_data(data_list: List[Dict[str, Any]]) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    fact_columns = [
        'stock_id', 'date_id', 'ticker_id', 'open', 'high', 'low', 
        'close', 'volume'
    ]
...
def load_to_rds(engine, df_fact_table: pd.DataFrame, df_dim_date: pd.DataFrame, df_dim_ticker: pd.DataFrame):
